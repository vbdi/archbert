...
from ppuda.ppuda.deepnets1m.graph import batch_graph_padding
...
class SentenceTransformer(nn.Sequential):
    ...
    def __init__(self, model_name_or_path: Optional[str] = None,
                 ...
                 max_node_size=512, max_batch_size=-1, mlm=False, mam=False
                 ):
        ...
        # ni
        #self.archModel = GHN(max_shape=(64, 64, 11, 11), num_classes=10, hypernet='gatedgnn', ve=0, hid=768, device='cuda', layernorm=False)#.train()
    ...
    def encode(self, sentences: Union[str, List[str]], arch_embeds=None,
               batch_size: int = 32,
        ...
        for start_index in trange(0, (len(sentences) if sentences else arch_embeds.shape[0]), batch_size, desc="Batches", disable=not show_progress_bar):
            if sentences:
                ...
                arch_embeds_batch = arch_embeds[start_index:start_index + batch_size]
                features['inputs_embeds'] = arch_embeds_batch
            ...
            with torch.no_grad():
                ...
                if output_value == 'token_embeddings':
                    embeddings = []
                    # ni
                    # out_features['attention_mask'] = torch.ones(out_features[output_value].shape)
                    if out_features['attention_mask'] is None:
    ...
    def save(self, path: str, model_name: Optional[str] = None, create_model_card: bool = True, loss_model=None):
        """
        ...
        # ni save other models (e.g., archModel, etc)
        if loss_model is not None:
            if loss_model.mam_head is not None:
                if isinstance(loss_model.mam_head, torch.nn.DataParallel):
                    torch.save(loss_model.mam_head.module, path + '/mam_head.pth')
                else:
                    torch.save(loss_model.mam_head, path + '/mam_head.pth')
            if loss_model.mlm_head is not None:
                if isinstance(loss_model.mlm_head, torch.nn.DataParallel):
                    torch.save(loss_model.mlm_head.module, path + '/mlm_head.pth')
                else:
                    torch.save(loss_model.mlm_head, path + '/mlm_head.pth')
            if hasattr(loss_model, 'archModel'):
                if isinstance(loss_model.archModel, torch.nn.DataParallel):
                    torch.save(loss_model.archModel.module, path + '/GHN.pth')
                else:
                    torch.save(loss_model.archModel, path + '/GHN.pth')
            if hasattr(loss_model, 'qaModel'):
                if isinstance(loss_model.qaModel, torch.nn.DataParallel):
                    torch.save(loss_model.qaModel.module, path + '/qaModel.pth')
                else:
                    torch.save(loss_model.qaModel, path + '/qaModel.pth')
            if hasattr(loss_model, 'langDec'):
                if isinstance(loss_model.langDec, torch.nn.DataParallel):
                    torch.save(loss_model.langDec.module, path + '/langDecModel.pth')
                else:
                    torch.save(loss_model.langDec, path + '/langDecModel.pth')
    ...
    # ni
    def mlm_mask_tokens(self, inputs, special_tokens_mask=None, mlm_probability=0.15):
        """
        ...
        import random
        random.seed(100)
        # the following only works for BS=1
        #if inputs.shape[1]<=5: # to make sure small inputs do not end up with all [-100] labels, which causes NAN loss!
        #    mlm_probability = mlm_probability * 2.0
        labels = inputs.clone()
    ...
    def mam_mask_tokens(self, inputs, mask_token, padd_token, na_vocab_size, mam_probability=0.15):
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        import torch
        ...
        labels = inputs.clone()
        # We sample a few tokens in each sequence for MLM training (with probability `self.mlm_probability`)
        probability_matrix = torch.full(labels.shape, mam_probability)

        special_tokens_mask = [[1 if token in [padd_token] else 0 for token in val] for val in labels.tolist()]
        special_tokens_mask = torch.tensor(special_tokens_mask, dtype=torch.bool)
        ...
        # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool() & masked_indices
        inputs[indices_replaced] = mask_token#self.tokenizer.convert_tokens_to_ids(mask_token)

        # 10% of the time, we replace masked input tokens with random word
        indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        random_words = torch.randint(na_vocab_size, labels.shape, dtype=torch.long).cuda()
        inputs[indices_random] = random_words[indices_random]
    ...
    def smart_batching_collate(self, batch):
        ...
        for example in batch:
            for idx, text in enumerate(example.texts):
                ...
                if example.label[idx] is not None:
                    labels.append(example.label[idx])
                # ni
                if isinstance(batch[0], InputExample):
                    # if model
                    if example.arch is not None:
                        archs.append(example.arch)#.to(self._target_device))
                    # if Graph
                    if example.graph is not None:
                        #graphs.append(example.graph)
                        node_feats.append(example.graph[0])
                        shape_inds.append(example.graph[1])
                        #if example.graph[2] is not None:
                        #    edges.append(example.graph[2])
                        adjs.append(example.graph[3])
                        #edges.append(example.graph.node_feat)

        # ni the following does not work for OURS dataset
        '''
        all_data = list(zip(node_feats, shape_inds, adjs, texts[0], labels))
        # we need the following to make sure, the retreived batch is the same in each epoch
        random.seed(10)
        random.shuffle(all_data)
        node_feats, shape_inds, adjs, texts[0], labels = zip(*all_data)
        node_feats, shape_inds, adjs, texts[0], labels = node_feats[:self.max_batch_size], shape_inds[:self.max_batch_size], adjs[:self.max_batch_size], texts[0][:self.max_batch_size], labels[:self.max_batch_size]
        '''

        ## batch-pad the graphs with maximum length in the batch
        node_feats, shape_inds, adjs, mask_token, padd_token, na_vocab_size = batch_graph_padding(list(node_feats), list(shape_inds), list(adjs),max_node_size=self.max_node_size,return_mask_id=True)

        #node_feats, shape_inds, adjs = batch_graph_padding(list(node_feats), list(shape_inds), list(adjs), max_node_size=self.max_node_size)
        #sa
        # sa: MAM
        '''
        if self.mam:            
            node_feats_nomask = node_feats
            node_feats = []
            #node_feats_array=node_feats.detach().numpy()
            random_percent = 0.15
            #node_feats = node_feats_nomask.data
            for ind_node_feat in np.arange(len(node_feats_nomask)):
                np.random.seed(100)
                random_inex =  np.random.choice(np.arange(node_feats_nomask[ind_node_feat].shape[0]), int(node_feats_nomask[ind_node_feat].shape[0]*random_percent))
                #I had issues with In-place operator and replacing the orignal values so I solved it by new names
                node_feat_here =  (node_feats_nomask[ind_node_feat]).clone().detach().numpy()
                node_feat_here[random_inex] = mask_id
                node_feats.append(torch.from_numpy(node_feat_here))

            node_feats, shape_inds, adjs, mask_id = batch_graph_padding(list(node_feats), list(shape_inds),list(adjs),max_node_size=self.max_node_size,return_mask_id=True)
        else:
            node_feats_nomask, shape_inds, adjs = batch_graph_padding(list(node_feats), list(shape_inds), list(adjs),
                                                           max_node_size=self.max_node_size, return_mask_id=False)
            node_feats = node_feats_nomask
        '''

        node_feats = torch.stack(node_feats).to(self._target_device)
        shape_inds = torch.stack(shape_inds).to(self._target_device)
        if len(edges)>0:
            edges = torch.stack(edges).to(self._target_device)
        if len(adjs)>0:
            adjs = torch.stack(adjs).to(self._target_device)

        labels = torch.tensor(labels).to(self._target_device)

        sentence_features = []
        for idx in range(num_texts):
            # ni need to pad to the size of max_node_size=i.e., otherwise, MLM with masks cannot be used for finetuning langDec
            tokenized = self.tokenize(texts[idx])
            if self.mlm:
                tokenized['input_ids'], tokenized['mlm_labels'] = self.mlm_mask_tokens(tokenized['input_ids'])
            batch_to_device(tokenized, self._target_device)
            sentence_features.append(tokenized)

        if not isinstance(batch[0], InputExample):
            graphs = batch

        # ni
        #graphs = GraphBatch(graphs, padding=self.graph_padding).to_device(self._target_device)

        if self.mam:
            node_feats, sentence_features[0]['mam_labels'] = self.mam_mask_tokens(inputs=node_feats, mask_token=mask_token, padd_token=padd_token, na_vocab_size=na_vocab_size)

        return sentence_features, labels, node_feats, shape_inds, edges, adjs, archs
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    features, labels, node_feats, shape_inds, edges, adjs, archs = data

                    if use_amp:
                        with autocast():
                            main_loss, mlm_loss, mam_loss, loss_value = loss_model(features, labels, node_feats, shape_inds, edges, adjs, archs)
                    ...
                    steps_loss_values.append(loss_value.item())
                    steps_main_loss.append(main_loss.item())
                    if torch.isnan(mlm_loss) == False:
                        steps_mlm_loss.append(mlm_loss.item())
                    if torch.isnan(mam_loss) == False:
                        steps_mam_loss.append(mam_loss.item())
                ...
                if checkpoint_path is not None and checkpoint_save_steps is not None and checkpoint_save_steps > 0 and global_step % checkpoint_save_steps == 0:
                    self._save_checkpoint(checkpoint_path, checkpoint_save_total_limit, global_step, loss_model)

            epoch_loss_value = np.mean(steps_loss_values)
            epoch_main_loss = np.mean(steps_main_loss)
            epoch_mlm_loss = np.mean(steps_mlm_loss)
            epoch_mam_loss = np.mean(steps_mam_loss)
            #epoch_loss_values.append(epoch_loss_value)
            print('Epoch average loss: %f' % (epoch_loss_value))
            print('Epoch average main_loss: %f' % (epoch_main_loss))
            print('Epoch average mlm_loss: %f' % (epoch_mlm_loss))
            print('Epoch average mam_loss: %f' % (epoch_mam_loss))

            self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback)

        if evaluator is None and output_path is not None:   #No evaluator, but output path: save final model version
            self.save(output_path, loss_model)
    ...
    def _save_checkpoint(self, checkpoint_path, checkpoint_save_total_limit, step, loss_model=None):
        # Store new checkpoint
        self.save(os.path.join(checkpoint_path, str(step)), loss_model=loss_model)
