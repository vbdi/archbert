...
def load_for_validation(modelpath, dataset, data_path, num_nets, batch_size, archModel, langModel, cross_encoder, max_node_size, max_edge_size, hypernet, checkpoint_epoch=None):
    ...
    if dataset=='tvhf':
        ...
        #graphs_list = tvhf_load_clone_ds(data_path + '/val_bcd.csv', load_path=data_path + '/*.graph', bimodal=True, num_nets=num_nets, shuffle=True)
...
def demo(modelpath, archModel, langModel, dataset, data_path, num_nets, batch_size, cross_encoder, max_node_size, max_edge_size, hypernet, query, checkpoint_epoch=None):
    ...
    if cross_encoder:
        arch_query_emb_pooled = langModel.encode(sentences=None, arch_embeds=arch_query_emb, batch_size=8, convert_to_tensor=True)  # , normalize_embeddings=True)
    ###################################
    ...
    if len(query)>0:
        ...
        query_scores = (util.cos_sim(query_emb, archs_emb_pooled))[0].cpu().tolist()
        # take the average score of (arch_query,archs) and (query,archs)
        scores = list((0.3*np.array(scores)+0.7*np.array(query_scores)))
    ####
...
def validate(modelpath, archModel, langModel, dataset, data_path, num_nets, batch_size, cross_encoder, max_node_size, max_edge_size, hypernet, arch_query, query, checkpoint_epoch=None):
    ### load graphs, archModel, and compute the embeddings
    #names, arch_emb_pooled = load_archModel_for_validation(modelpath, archModel, langModel, cross_encoder, max_node_size, max_edge_size)
    ###
    ...
    arch_query_emb, arch_query_emb_pooled = archModel(node_feats=arch_query_node_feat, shape_inds=arch_query_shape_ind, edges=None, adjs=arch_query_Adj)
    if cross_encoder:
        arch_query_emb_pooled = langModel.encode(sentences=None, arch_embeds=arch_query_emb, batch_size=batch_size, convert_to_tensor=True)  # , normalize_embeddings=True)
    ###################################
